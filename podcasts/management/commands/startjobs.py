...
def save_new_episodes(feed):
    """Save new episodes to the database.
    Checks the episode GUID against the episodes currently 
    stored in the databes. If not fond, then a new Episode
...
def fetch_customprotocole_episodes():
    _feed = feedparser.parse('https://www.customprotocol.com/feed/')
    save_new_episodes(_feed)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            fetch_customprotocole_episodes,
            trigger='interval',
            minutes=2,
            id='Custom Protocole',
            max_instances=1,
        ...
        logger.info('Add job: Custom Protocole Feed')
